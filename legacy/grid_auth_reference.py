...
def executeCommand(command, timeout=None):
    ...
    try:
        ...
        logging.warning(
            'Warning: an error occurred closing subprocess [%s] %s  %s' %
...
class Proxy(object):
    ...
    def checkMyProxy(self, proxy=None, Time=4, checkRetrieverRenewer=False):
        ...
        if not out:
            self.logging.info(
                'No credential delegated to myproxy server %s .' %
                self.myproxyServer)
            valid = False
        else:
            ...
            timeleftList = re.compile(
                "timeleft: (?P<hours>[\\d]*):(?P<minutes>[\\d]*):(?P<seconds>[\\d]*)").findall(out)

            # the first time refers to the flat user proxy, the other ones are
            # related to the server credential name
            try:
            ...
            if checkRetrieverRenewer and len(self.serverDN.strip()) > 0:
                ...
                if serverCredName not in credNameList:
                    self.logging.info(
                        'Your proxy lacks of retrieval and renewal policies for the requested server.')
                ...
                for credIdx in range(len(credNameList)):
                    ...
                    timeleft = int(hours) * 3600 + \
                        int(minutes) * 60 + int(seconds)
                    if timeleft == 0:
                        cleanupCmd = "myproxy-destroy -d -k %s" % (
                            credNameList[credIdx])
                ...
                if len(cleanCredCmd) > 0:
                    self.logging.debug(
                        'Removing expired credentials: %s' %
                        cleanCredCmd)
                    try:
    ...
    def ManualRenewMyProxy(self):
        ...
        if len(self.serverDN.strip()) > 0:
            ...
            cmd += ' -x -R \'%s\' -Z \'%s\' -k %s -t 168:00 ' % (
                self.serverDN, self.serverDN, credName)
    ...
    def logonMyProxy(
        ...
        # myproxy-logon -d -n -s $MYPROXY_SERVER -o <outputFile> -l <userDN> -k
        # <credName>
        ...
        credName = sha1(self.getSubject(
            '$HOME/.globus/hostcert.pem')).hexdigest()

        # compose the delegation or renewal commands with the regeneration of
        # Voms extensions
        cmdList = []
        ...
        if (out > 0):
            ...
            raise Exception(
                "Unable to retrieve delegated proxy for user DN %s! Exit code:%s" %
                (userDN, out))
    ...
    def renewalMyProxy(self, proxyFilename):
        ...
        if ret != 0:
            raise Exception(
                "Unable to get VO for proxy %s! Exit code:%s" %
                (proxyFilename, ret))
        vo = att.replace('\n', '')
        ...
        if ret != 0:
            raise Exception(
                "Unable to get FQAN for proxy %s! Exit code:%s" %
                (proxyFilename, ret))
        ...
        cmdList.append(
            'myproxy-logon -d -n -s %s -a %s -o %s -k %s -t 168:00' %
        ...
        if (out > 0):
            ...
            raise Exception(
                "Unable to retrieve proxy for renewal: %s! Exit code:%s" %
                (proxyFilename, out))
    ...
    def vomsExtensionRenewal(self, proxy, voAttr='cms'):
        ...
        if ret != 0 and ret != 1:
            raise Exception(
                "Error while checking retrieved proxy timeleft for %s" %
                proxy)
        ...
        cmdList.append(
            'voms-proxy-init -noregen -voms %s -cert %s -key %s -out %s -bits 1024 -valid %s' %
...
class GridMapChecker(object):
    ...
    def load(cls, gridMapFile):
        ...
        for line in fd.readlines():
            ...
            try:
                l = line.split('"')[
                    1:]  # not interested in the first empty string
                # TODO
                # improve
    ...
    def requestLocalId(self, subject):
        ...
        if not user:
            ...
            print (
                "mapping match found: subject: '%s' gridmapfile user: '%s'" %
                (subject, user))
        return user
