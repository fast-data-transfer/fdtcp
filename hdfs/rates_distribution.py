...
for line in fd:
    ...
    # transfer-fdtcp-cithep249.ultralight.org--srm.unl.edu-to-gridftp05.ultralight.org--2011-04-24-01h:09m:42s-bqtsq.log
    # 21.13 MB/s
    rate = line.split()[1]
    rate = float(rate)
...
print(
    "total number of binned values: %s (must be the same as number of input values)" %
    numBinned)
...
pylab.title(
    "Transfer performance distribution over %s measurements" %
    len(ratesList))
pylab.grid(True)
